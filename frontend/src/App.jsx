...
export default function App() {
  ...
  const [memberDataTab, setMemberDataTab] = useState("members");
  ...
  const masterDataSection = {
    ...
    fields: [
      ...
      { name: "patta_name", label: "Nick Name (பட்டப்பெயர்)", required: true },
      ...
      { name: "pincode", label: "Pincode", required: true },
      {
        ...
        required: true,
        options: [
          { value: "Nerkuppai", label: "Nerkuppai (நெற்குப்பை)" },
          { value: "Venthanpatti", label: "Venthanpatti (வேந்தன்பட்டி)" },
        ],
      },
    ],
    columns: [
    ...
    detailFields: [
      { key: "member_id", label: "Member ID" },
      { key: "name", label: "Name" },
      { key: "patta_name", label: "Nick Name (பட்டப்பெயர்)" },
      { key: "primary_phone", label: "Primary Phone" },
  ...
  const nonMasterDataSection = {
    ...
    endpoint: "relatives",
    rowKey: "id",
    description: "Non-member relationship register for guests and Penn Veetaar entries. Search by non-member ID, name, or phone number.",
    ...
    fields: [
      ...
      { name: "phone_2", label: "Secondary Phone" },
      {
        ...
        required: true,
        options: [
          { value: "Guest", label: "Guest (விருந்தினர்)" },
          { value: "Penn Veetaar", label: "Penn Veetaar (பெண் வீட்டார்)" },
        ],
      },
      {
        name: "native_place",
    ...
    columns: [
      ...
      { key: "type", label: "Type", render: (row) => ({ Guest: "Guest (விருந்தினர்)", "Penn Veetaar": "Penn Veetaar (பெண் வீட்டார்)" }[row.type] ?? row.type) },
      { key: "native_place", label: "Native Place", render: (row) => ({ Nerkuppai: "Nerkuppai (நெற்குப்பை)", Venthanpatti: "Venthanpatti (வேந்தன்பட்டி)" }[row.native_place] ?? row.native_place ?? "-") },
    ],
    detailFields: [
      { key: "non_member_id", label: "Non member ID" },
      { key: "name", label: "Name" },
  ...
  const sections = [
    ...
    },
    {
      ...
      columns: [
        ...
        { key: "price", label: "Price (Rs.)", align: "right", render: (row) => amount(row.price) },
        { key: "payment_status", label: "Payment Status", align: "center", render: (row) => paymentStatusBadge(row.payment_status) },
        ...
        {isDashboardView ? (
          ...
          <section className="module-stage">
            ...
            {activeSection.nestedSections ? (
              <>
                <div className="member-data-tabs">
                  {activeSection.nestedSections.map((section) => (
                    <button
                      key={section.endpoint}
                      type="button"
                      className={`member-data-tab ${
                        memberDataTab === section.endpoint ? "member-data-tab--active" : ""
                      }`}
                      onClick={() => setMemberDataTab(section.endpoint)}
                    >
                      {section.title}
                    </button>
                  ))}
                ...
                {activeSection.nestedSections
                  .filter((section) => section.endpoint === memberDataTab)
                  .map((section) => (
                    <ResourceSection
                      key={section.endpoint}
                      config={section}
