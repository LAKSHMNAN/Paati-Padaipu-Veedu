...
class Member(TimeStampedModel):
    class NativePlace(models.TextChoices):
        ...
        VENDANPATTI = "Venthanpatti", "Venthanpatti"
    ...
    secondary_phone = models.CharField(max_length=10, blank=True, null=True, validators=[phone_validator])
    ...
    @classmethod
    def _next_member_id(cls):
        existing_ids = (
            cls.objects
            .filter(member_id__startswith="MEM_")
            .values_list("member_id", flat=True)
        )
        used_numbers = []
        for mid in existing_ids:
            try:
                used_numbers.append(int(str(mid).rsplit("_", 1)[1]))
            except (IndexError, ValueError):
                pass
        next_number = max(used_numbers, default=0) + 1
        return f"MEM_{next_number:03d}"
    ...
    def save(self, *args, **kwargs):
        if not self.member_id:
            self.member_id = self._next_member_id()
        self.full_clean()
...
class Relative(TimeStampedModel):
    class RelativeType(models.TextChoices):
        GUEST = "Guest", "Guest"
        PENN_VITARR = "Penn Veetaar", "Penn Veetaar"
    ...
    phone_2 = models.CharField(max_length=10, blank=True, null=True, validators=[phone_validator])
    ...
    def clean(self):
        ...
        if self.phone_2 and phone_exists_elsewhere(Relative, ["phone_1"], self.phone_2, self.pk):
            raise ValidationError({"phone_2": "This phone number is already used as a primary phone."})
