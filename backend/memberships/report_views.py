...
class AuctionTransactionByItemReportView(View):
    """Report grouped by auction item with paid/unpaid breakdown per item."""

    def get_report_data(self):
        from .models import AuctionItem

        items = AuctionItem.objects.all().order_by("auction_item_name")

        result_items = []
        grand_total = Decimal("0.00")
        grand_paid = Decimal("0.00")
        grand_unpaid = Decimal("0.00")
        grand_total_count = 0
        grand_paid_count = 0
        grand_unpaid_count = 0

        for item in items:
            txns = AuctionTransaction.objects.filter(item=item)
            agg = txns.aggregate(
            ...
            unpaid_amount = agg["unpaid_amount"] or Decimal("0.00")

            grand_total += total_amount
            grand_paid += paid_amount
            grand_unpaid += unpaid_amount
            grand_total_count += agg["total_count"] or 0
            grand_paid_count += agg["paid_count"] or 0
            grand_unpaid_count += agg["unpaid_count"] or 0

            result_items.append({
                "item_id": item.id,
                "item_name": item.auction_item_name,
                "item_name_tamil": item.auction_item_name_tamil or "",
        ...
        summary = {
            ...
            "total_transactions": grand_total_count,
            "paid_count": grand_paid_count,
            "unpaid_count": grand_unpaid_count,
            "total_amount": grand_total,
            "paid_amount": grand_paid,
            "unpaid_amount": grand_unpaid,
            "report_date": datetime.now(),
        }
    ...
    def get(self, request, *args, **kwargs):
        ...
        if format_type == "json":
            ...
            return HttpResponse(
                json.dumps({"error": "Invalid format. Supported: json, csv, excel"}),
                content_type="application/json",
                status=400,
            )
    ...
    def get_json_report(self):
        data = self.get_report_data()
        return HttpResponse(
            json.dumps({"summary": data["summary"], "items": data["items"]}, default=str),
            content_type="application/json",
            status=200,
        )
    ...
    def get_csv_report(self):
        ...
        writer.writerow(["AUCTION TRANSACTION REPORT BY ITEM"])
        writer.writerow(["Generated Date", summary["report_date"]])
        writer.writerow([])
        writer.writerow(["SUMMARY"])
        writer.writerow(["Total Auction Items", summary["total_items"]])
        writer.writerow(["Total Transactions", summary["total_transactions"]])
        writer.writerow(["Paid Count", summary["paid_count"]])
        writer.writerow(["Unpaid Count", summary["unpaid_count"]])
        writer.writerow(["Total Amount", f"Rs. {summary['total_amount']}"])
        writer.writerow(["Paid Amount", f"Rs. {summary['paid_amount']}"])
        writer.writerow(["Unpaid Amount", f"Rs. {summary['unpaid_amount']}"])
        writer.writerow([])
        writer.writerow(["BREAKDOWN BY AUCTION ITEM"])
        writer.writerow([
            "Auction Item", "Total Tokens", "Paid", "Unpaid",
        ...
        for row in items:
            writer.writerow([
                ...
                f"Rs. {row['paid_amount']}",
                f"Rs. {row['unpaid_amount']}",
            ])

        response = HttpResponse(output.getvalue(), content_type="text/csv")
        response["Content-Disposition"] = 'attachment; filename="auction_by_item_report.csv"'
        return response
    ...
    def get_excel_report(self):
        try:
            ...
            return HttpResponse(
                json.dumps({"error": "openpyxl not installed."}),
                content_type="application/json",
                status=400,
            )
        ...
        worksheet = workbook.active
        worksheet.title = "By Auction Item"
        ...
        title_cell = worksheet["A1"]
        title_cell.value = "AUCTION TRANSACTION REPORT BY ITEM"
        title_cell.font = title_font
        title_cell.fill = title_fill
        title_cell.alignment = workbook._report_center_alignment
        worksheet.row_dimensions[1].height = 28
        ...
        summary_rows = [
            ("Total Auction Items", summary["total_items"]),
            ("Total Transactions", summary["total_transactions"]),
            ("Paid Count", summary["paid_count"]),
            ("Unpaid Count", summary["unpaid_count"]),
            ("Total Amount", money_decimal(summary["total_amount"])),
            ("Paid Amount", money_decimal(summary["paid_amount"])),
            ("Unpaid Amount", money_decimal(summary["unpaid_amount"])),
        ]
        money_labels = {"Total Amount", "Paid Amount", "Unpaid Amount"}
        for idx, (label, value) in enumerate(summary_rows):
            ...
            worksheet.cell(row=row, column=1).fill = section_fill
            worksheet.merge_cells(start_row=row, start_column=2, end_row=row, end_column=7)
            val_cell = worksheet.cell(row=row, column=2)
            val_cell.value = value
            if label in money_labels:
                val_cell.number_format = '"Rs." #,##0.00'
            if "Paid" in label and "Unpaid" not in label:
                val_cell.fill = positive_fill
            elif "Unpaid" in label:
                val_cell.fill = warning_fill
            for col in range(1, 8):
                worksheet.cell(row=row, column=col).border = border
                worksheet.cell(row=row, column=col).alignment = workbook._report_body_alignment

        # Item breakdown table
        header_row = 5 + len(summary_rows) + 2
        worksheet.merge_cells(start_row=header_row - 1, start_column=1, end_row=header_row - 1, end_column=7)
        worksheet.cell(row=header_row - 1, column=1).value = "Breakdown by Auction Item"
        worksheet.cell(row=header_row - 1, column=1).font = section_font
        worksheet.cell(row=header_row - 1, column=1).fill = section_fill

        columns = ["Auction Item", "Total Tokens", "Paid", "Unpaid", "Total Amount (Rs.)", "Paid Amount (Rs.)", "Unpaid Amount (Rs.)"]
        for col, header in enumerate(columns, 1):
            ...
            cell.font = header_font
            cell.border = border
            cell.alignment = workbook._report_center_alignment

        money_cols = {5, 6, 7}
        for item in items:
            row = header_row + 1 + items.index(item)
            values = [
                item["item_name"],
                item["total_count"],
                item["paid_count"],
                item["unpaid_count"],
                money_decimal(item["total_amount"]),
                money_decimal(item["paid_amount"]),
                money_decimal(item["unpaid_amount"]),
            ]
            for col, value in enumerate(values, 1):
                cell = worksheet.cell(row=row, column=col)
                cell.value = value
                cell.border = border
                cell.alignment = workbook._report_body_alignment
                if col in money_cols:
                    cell.number_format = '"Rs." #,##0.00'

        last_data_row = header_row + len(items)
        worksheet.auto_filter.ref = f"A{header_row}:G{last_data_row}"

        for col_letter, width in {"A": 32, "B": 14, "C": 12, "D": 12, "E": 20, "F": 20, "G": 20}.items():
        ...
        return build_excel_response(workbook, "auction_by_item_report.xlsx")


class AuctionTransactionItemDetailView(View):
    """Return paginated transactions for a single auction item, with summary."""
    ...
    def get(self, request, item_id, *args, **kwargs):
        ...
        try:
            ...
            return HttpResponse(
                json.dumps({"error": "Auction item not found."}),
                content_type="application/json",
                status=404,
            )

        qs = AuctionTransaction.objects.select_related(
            "member", "relative", "item", "receipt"
        ).filter(item=item)
        ...
        payload = {
            "summary": summary,
            "transactions": AuctionTransactionDetailReportSerializer(paged, many=True).data,
            "pagination": {
                "count": total_count,
                "limit": limit,
                "offset": offset,
