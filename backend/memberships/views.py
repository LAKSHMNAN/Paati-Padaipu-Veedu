...
def get_account_id_from_request(request):
    """Read account_id from session cookie or X-Session-Key header (for cross-origin dev)."""
...
class LoginView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({"user": RegistrationSerializer(account).data, "session_key": request.session.session_key}, status=status.HTTP_200_OK)
...
class LogoutView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if session_key:
            ...
            try:
                store = SessionStore(session_key)
                store.pop(AUTH_SESSION_KEY, None)
                store.save()
            except Exception:
                pass
